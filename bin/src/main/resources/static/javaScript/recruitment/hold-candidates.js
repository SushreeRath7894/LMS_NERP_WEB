$(document).ready(function() {
	...
	new agGrid.Grid(gridDivSelected, gridOptionsHold);
...
function getHoldCandidateList() {
	agGrid.simpleHttpRequest({
		url: "review-hiring-hold-candidates",
	}).then(function(data) {
		...
		if (jsonData && Array.isArray(jsonData) && jsonData.length > 0) {
			...
			if (jsonData.length > 0) {
				gridOptionsHold.api.forEachNode(function(node) {
					if (node.rowIndex === 0) {
