...
function getPaymenDetails(patId) {
	...
	if(selectedRows && selectedRows.length > 0) {
		let a = selectedRows[0];
		$(bookingIds).text(a?.bookingId || "N/A");
		$(grandTotal).text(a?.grandTotal?.toFixed(2) || "0.00");
		$("#pendingAmount").text(a?.pendingAmount?.toFixed(2) || "0.00");
		pendingAmnt = a?.pendingAmount;
		if(a?.pendingAmount == 0) {
			$("#payBtn1").addClass("d-none");
		} else {
			$("#payBtn1").removeClass("d-none");
		}
	} else {
		$(bookingIds).text("N/A");
...
function initiateBillingProcess() {
	...
	if(selectedRows && selectedRows[0]) {
		...
		custId = selectedRows[0]?.patientId;
