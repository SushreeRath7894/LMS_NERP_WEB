...
$(document).ready(function() {
	...
	/*agGrid.simpleHttpRequest({
		...
		var allData = jsonData.CreditNoteData;
		console.log(allData)
...
function getOrderNUmber(id) {
	if (id) {
		...
		$.ajax({
			type: "GET",
			url: "credit-note-getorderList?id=" + id,
			success: function(response) {
				if (response.message == "success") {
					$("#orderNumber").empty();
					var option = $("<option></option>");
...
function getOrderNUmber2(id1, id2) {
	if (id1) {
		...
		$.ajax({
			type: "GET",
			url: "credit-note-getorderList?id=" + id1,
			success: function(response) {
				if (response.message == "success") {
					$("#orderNumber").empty();
					var option = $("<option></option>");
...
function getProductList() {
	...
	if (ord) {
		...
		$.ajax({
			type: "GET",
			url: "credit-note-getProductList?id=" + ord,
			success: function(response) {
				if (response.message == "success") {
					$("#quantity").val("");
					$("#priceRate").val("");
...
function getProductDetails() {
	...
	$.ajax({
		type: "GET",
		url: "credit-note-getProductDetails?oid=" + ord + "&proid=" + proid,
		success: function(response) {
			if (response.message == "Success") {
				$("#quantity").val(response.body[0].quantity);
				$("#priceRate").val(response.body[0].itemUnitPrice);
				$("#productMode").val(response.body[0].toggleRegularCustom);
				...
				if (response.body[0].extraDiscount != '') {
					$("#extraDiscount").val(response.body[0].extraDiscount);
				} else {
					$("#extraDiscount").val('0');
				}
...
var columnDefs = [
...

	{
		...
		/*cellRenderer: function(params) {
			return '<a onclick=editCreditNOte("' + params.data.creditNoteId
	...
	},
	{
		...
		headerName: "Credit Date(dd-mm-yyyy)",
...
var productDefs = [
	...
	},
	{
		...
		//valueFormatter : currencyFormatter
		cellRenderer: function(params) {
			const formattedValue = parseFloat(params.data.itemCgst).toFixed(2);
			return '<div>' +
				formattedValue +
				'</div>';
		}
		...
		//valueFormatter : currencyFormatter
		cellRenderer: function(params) {
			const formattedValue = parseFloat(params.data.itemSgst).toFixed(2);
			return '<div>' +
				formattedValue +
				'</div>';
		}
		...
		//valueFormatter : currencyFormatter
		cellRenderer: function(params) {
			const formattedValue = parseFloat(params.data.itemIgst).toFixed(2);
			return '<div>' +
				formattedValue +
				'</div>';
		}
...
function onSelectionChanged() {
	...
	if (rowCount > 0) {
		var crNoteId = selectedRows[0].creditNoteId;
		$('#reqDltBtn').attr('disabled', false);
		$('#newBtn').attr('disabled', true);
		$('#dwnldBtn').attr('disabled', false);
		editCreditNOte(crNoteId);
		$("#add-btn").removeClass("d-none");
		$("#ticket-editBtn").removeClass("d-none");
		$("#credit-note-dlt-btn").removeClass("d-none");
		$(".credit-note-creation-cntr").addClass("textareadisable");
		...
		$("#credit-note-cancel-btn").addClass("d-none");
		$("#credit-note-save-btn").addClass("d-none");
		...
		$("#credit-note-dlt-btn").addClass("d-none");
...
function creditNoteContainer() {
	$("#journalVoucher").text('');
	$(".credit-note-creation-cntr").removeClass("textareadisable");
	...
	$("#credit-note-save-btn").removeClass("d-none");
	$("#credit-note-cancel-btn").removeClass("d-none");
	$("#ticket-editBtn").addClass("d-none");
	CKEDITOR.instances['descriptionCredit'].setData('');
...
function editProduct(slNo) {
	...
	$("#returnQuantity").val(rowNode.data.quantity);
	$("#unit").val(rowNode.data.unit);
	$("#priceRate").val(rowNode.data.itemUnitPrice);
	$("#gstRate").val(rowNode.data.gstRate);
	$("#skuTemp").val(rowNode.data.sku);
	$("#totalAmount").val(rowNode.data.lineTotal);
	$("#extraDiscount").val(rowNode.data.extraDiscount);
...
function editCreditNOte(id) {
	...
	//$("#asAvoucherBtn").hide();
	$(".innercontent").show();
	$("#journalVoucher").text(id);


	$.ajax({
		type: "GET",
		url: "credit-note-editVoucher?id=" + id,
		success: function(response) {
...
function deleteOnclick() {
	...
	$.ajax({
		type: "GET",
		url: "credit-note-delete-id?id=" + deletedId,
		success: function(response) {
			if (response.code == "Success") {
				//cancelBtn();
				toastr.success("Credit note deleted successfully.")
				viewFilteredData();
...
function saveTableData() {
	...
	if (validation) {
		...
		item.itemUnitPrice = $('#priceRate').val();
		item.discount = $('#discount').val();
		item.dealerCode = $('#discountCode').val();
		item.gstRate = $('#gstRate').val();
		item.lineTotal = $('#totalAmount').val();
		item.categoryName = $("#proName").val();
		item.categoryId = $('#itemName').val();
		item.productDimension = $('#productSize').val();
		item.toggleRegularCustom = $('#productMode').val();
		item.extraDiscount = $('#extraDiscount').val();
...
function saveData() {
	...
	setTimeout(function() {
		...
		productOptions.api.forEachNode(function(rowNode, indrx) {
			var item_obj = {};
			item_obj.productId = rowNode.data.productId;
			item_obj.productName = rowNode.data.productName;
			item_obj.categoryId = rowNode.data.categoryId;
			item_obj.categoryName = rowNode.data.categoryName;
			item_obj.quantity = rowNode.data.quantity;
			item_obj.productDimension = rowNode.data.productDimension;
			item_obj.itemUnitPrice = rowNode.data.itemUnitPrice;
			item_obj.discount = rowNode.data.discount;
			item_obj.gstRate = rowNode.data.gstRate;
			item_obj.lineTotal = rowNode.data.lineTotal;
			item_obj.itemCgst = rowNode.data.itemCgst;
			item_obj.itemSgst = rowNode.data.itemSgst;
			item_obj.itemIgst = rowNode.data.itemIgst;
			item_obj.taxableAmt = rowNode.data.taxableAmt;
			item_obj.dealerCode = rowNode.data.dealerCode;
			item_obj.extraDiscount = rowNode.data.extraDiscount;
			item_obj.toggleRegularCustom = rowNode.data.toggleRegularCustom;
		...
		success: function(response) {
			if (response.code == "Success") {
				...
				/*swal({
					...
					text: "Credit voucher created Successfully!",
					type: "success"
				}).then(function() {
					location.reload();
				}*/

				toastr.success("Credit voucher created Successfully!")
...
function viewFilteredData() {
	...
	if (validation) {
		agGrid.simpleHttpRequest({
			url: "credit-note-view-filteredData?fromDate=" + fromDateFilter + "&toDate=" + toDateFilter,
		}).then(function(data) {
			$('.loader').hide();
...
//Function for adding more Table row For Debit Part
...
//Function for adding more Table row for credit
...
function SaveFinalAmount() {
	...
	else if ($("#modAdjustment").val() == "advance" || $("#modAdjustment").val() == "newRef" || $("#onAcc").val() == "advance") {
...
/*function autocompleteValue1(sGroupId, sGroupName, id, amount, taxType, multiRcvType, vendorId) {
	var l = id.split("_");
...
function submitJournal(dataset) {
	swal.fire({
		...
		if (result.value) {
			$.ajax({
				type: "POST",
				url: "credit-note-addCreditNoteVoucher",
				dataType: "json",
				contentType: "application/json",
				...
				success: function(response) {
					...
					if (response.message == "Success") {
						...
						toastr.success("Credit Note voucher created successfully.");
						setTimeout(() => {
							viewFilteredData();
						}, 1000);
						$("#credit-note-save-btn").addClass("d-none");
						/*	swal({
								title: "Voucher Status",
								text: "Credit Note voucher created successfully.",
								type: "success",
							}).then(function() {
								window.location.href = "/account/credit-note";
							})*/
					} else {
...
function onQuickFilterChanged() {
	...
	console.log("TvisibleRows:", visibleRows);

	const totalSum = visibleRows.reduce((sum, item) => sum + Number(item.grandTotal), 0);
...
function isDuplicateLedger(newLedgerName, newLedgerId, type) {
	...
	if (type == "debitType") {
		...
		console.log("in debit type check")
		...
		console.log("in credit type check")
		$('.creditAccountSubGroupCls').each(function() {
			let ledgerName = $(this).val().trim();
