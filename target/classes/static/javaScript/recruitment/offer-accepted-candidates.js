...
function rowSelectId() {
	var selectedNodes = gridOptionsOfferAccepted.api.getSelectedNodes();
	...
	var selectedRows = gridOptionsOfferAccepted.api.getSelectedRows();
	var rowCount = 0;
...
function getOfferAcceptedCandidateList() {
	agGrid.simpleHttpRequest({
		url: "review-hiring-accepted-candidates",
	}).then(function(data) {
		...
		if (jsonData && Array.isArray(jsonData) && jsonData.length > 0) {
			...
			if (jsonData.length > 0) {
				gridOptionsOfferAccepted.api.forEachNode(function(node) {
					if (node.rowIndex === 0) {
