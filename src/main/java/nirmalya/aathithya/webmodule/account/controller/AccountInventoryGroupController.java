...
public class AccountInventoryGroupController {
	Logger logger = LoggerFactory.getLogger(AccountInventoryGroupController.class);
	@Autowired
	...
	@GetMapping("manage-inventory-group")
	public String viewAccountGroup(Model model, HttpSession session) {
		...
		try {
			respTblact = restClient.getForObject(
					env.getAccountUrl() + "getAccountInventoryTreeDetails?Action=" + "getAccountInventoryTreeDetails" + "&orgName=" + orgName + "&orgDiv=" + orgDiv,
					JsonResponse.class);
		...
		return "account/manage-inventory-group";
	}

//view-inventory-group-add-parent

	@SuppressWarnings("unchecked")
	@RequestMapping(value = "view-inventory-group-add-parent", method = { RequestMethod.POST })
	public @ResponseBody JsonResponse<Object> addAccountParent(@ModelAttribute DataSetAccountTree dataForParent,
		...
		try {
			response = restClient.postForObject(env.getAccountUrl() + "restAddParentforInvntory", dataForParent,
					JsonResponse.class);
		} catch (RestClientException e) {
			e.printStackTrace();
		}
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "view-inventory-group-add-child", method = { RequestMethod.POST })
	public @ResponseBody JsonResponse<Object> addAccountChild(@ModelAttribute DataSetAccountTree dataForChild,
		...
		try {
			response = restClient.postForObject(env.getAccountUrl() + "restAddChildForInventory", dataForChild, JsonResponse.class);
		} catch (RestClientException e) {
			e.printStackTrace();
		}
	...
	@SuppressWarnings("unchecked")
	@GetMapping("view-inventory-group-modifyName")
	public @ResponseBody JsonResponse<Object> modifyHeaderName(@RequestParam String id, String nameGroup,
		...
		try {
			res = restClient.getForObject(env.getAccountUrl() + "modifyHeaderNameForInventory?id="+id + "&nameGroup=" + nameGroup + "&orgName=" + orgName + "&orgDiv=" + orgDiv, JsonResponse.class);
		} catch (RestClientException e) {
			e.printStackTrace();
		}
	...
	@SuppressWarnings("unchecked")
	@GetMapping("view-inventory-group-modifyNameParent")
	public @ResponseBody JsonResponse<Object> modifyHeaderNameParent(@RequestParam String id, String nameGroup, String natureGrp,
		...
		try {
			res = restClient.getForObject(env.getAccountUrl() + "modifyHeaderNameParentForInventory?id="+ id + "&nameGroup=" + nameGroup + "&natureGrp=" + natureGrp + "&orgName=" + orgName + "&orgDiv=" + orgDiv, JsonResponse.class);
		} catch (RestClientException e) {
			e.printStackTrace();
		}
